...
class SimulationBox(BaseModel):
    ...
    def __eq__(self, other: SimulationBox) -> bool:
        return all(
            [self.__dict__[key] == other.__dict__[key] for key in self.__fields_set__]
        )
...
class DumpSnapshot(BaseModel):
    ...
    def num_atoms_must_match_natoms(cls, v: List[Atom], values: dict, **kwargs):
        if len(v) != values["natoms"]:
            raise AssertionError(
                f"Number of atoms read from file does not match {values['natoms']}"
            )
        return v
    ...
    def __str__(self):
        ...
        simbox_header = (
            f"ITEM: BOX BOUNDS {self.box.xprd} {self.box.yprd} {self.box.zprd}"
        )
        atoms_header = "ITEM: ATOMS " + " ".join(
            sorted(
                [
                    colname
                    for colname in self.atoms[0].__fields_set__
                    if colname != "unwrapped"
                ]
            )
        )
    ...
    def dataframe(self) -> pd.DataFrame:
        return pd.DataFrame.from_dict(
            [atom.dict(exclude_unset=True) for atom in self.atoms]
        )
    ...
    def parse_snapshot(self):
        ...
        if natoms:
            ...
            for _ in range(0, natoms):
                row = {}
                for cname, value in zip(
                    column_names, self.file.readline().split()
                ):  # +natoms times
